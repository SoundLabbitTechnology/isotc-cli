...
| `config set provider gemini` | ✅ 成功 | .spec/config.toml 更新 | 0 |
| `config show --format json` | ✅ 成功 | 準拠 | 0 |
| `intent "..." --format json` | ⚠️ LLM 必須 | OPENAI/Gemini/Claude API キー必要 | - |
...
| **config** | ✅ 実装済 | set/show/list-providers | .spec/config.toml | 0/1 | 非公式 | .spec 欠損 | LLM プロバイダー・モデル設定 |
| **intent** | ✅ 実装済* | 要件テキスト, --file | requirements.json, open_questions.md, assumptions.toml | 0/1 | requirements.schema.json | LLM API キー欠損 | *OpenAI/Gemini/Claude、E2E 未カバー |
...
| doctor --format json | 出力形式固定（LLM プロバイダー別 API キー確認） |
| emit * --format json | 出力形式固定 |
| config show --format json | 出力形式固定 |
