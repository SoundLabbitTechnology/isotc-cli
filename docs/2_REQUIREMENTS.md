...
| `intent` | **実装済** | LLM による構造化、3ファイル出力、clarification loop |
| `plan` | **実装済** | 設計成果物一式、open_questions チェック、--force |
| `impl` | **部分実装** | `--isolated-prompt` で隔離プロンプト生成のみ対応 |
| `verify` | **実装済** | import/re-export/パッケージ/循環依存、JSON/Text、repairPrompt |
