...
| `impl` | 指定タスクの隔離環境での実装（AIエージェントへの委譲） | `--task-id <ID>`, `--agent <aider\|cline>` | 実装完了のステータス |
...
```
isotc impl --task-id <ID> [--agent aider|cline]
```
...
| `intent` | 未実装 | - |
| `plan` | 未実装 | - |
| `impl` | 未実装 | `auto_fix.sh` で Aider 連携のプロトタイプあり |
| `verify` | **部分実装** | AST解析・RuleValidator の骨格あり。JSON出力・終了コードは要実装 |
